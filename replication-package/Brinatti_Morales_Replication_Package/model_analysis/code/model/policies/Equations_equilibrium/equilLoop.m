...
%     PP - parameter struct.
%     SS - heterogeneity struct (firm draws; includes f_j_orig / f_j_nt_orig).
%
%   Outputs:
%
%     F                - 10x1 residual of the equilibrium conditions.
%     main_aggregates  - 1x18 vector of aggregate quantities (last element: subsidy_fixed).
...
f_j_nt      = SS.f_j_nt;
f_j_orig    = SS.f_j_orig;
f_j_nt_orig = SS.f_j_nt_orig;

% Per-firm policy subsidy. No transfer in the policies studied in the paper
transfer = 0;
...
% exporter in T (hirin immigrants)
 ...
 if max(check_nexp_nt)>0 || max(check_nexp_t)>0 || max(check_exp_t)>0
        ...
        RoW             = Inf*zeros(N_g, 5);
        main_aggregates = Inf*zeros(1, 18);
...
[prof_nexp_imm_nt,~,~]    = profits(sd_nexp_nt,Pd,Pd_nt,Px,Yd_t,Yd_nt,Yx,wd,wd_nt,wimm,wimm_nt,PP,SS, transfer);
...
[~,prof_nexp_imm,~]   = profits(sd_nexp,Pd,Pd_nt,Px,Yd_t,Yd_nt,Yx,wd,wd_nt,wimm,wimm_nt,PP,SS, transfer);
...
[~,~,prof_exp_imm]  = profits(sd_exp,Pd,Pd_nt,Px,Yd_t,Yd_nt,Yx,wd,wd_nt,wimm,wimm_nt,PP,SS, transfer);
...
[prof_nexp_nimm_nt,prof_nexp_nimm,prof_exp_nimm] = profitsnimm(ones(N_g,1),Pd,Pd_nt,Px,Yd_t,Yd_nt,Yx,wd,wd_nt,wimm,wimm_nt,PP,SS, transfer);
...
% Policy variant: indicator vectors of length N_g used to broadcast the per-firm
% transfer in the Ld_alt / Ld_alt_nt aggregation below. Currently transfer = 0.
D_all_t  = ones(size(D_exporter,1),1);
...
D_all_nt = ones(size(D_imm_nt,1),1);
...
% Policy variant: the 19th GER column / 16th GER_nt column (`f_j_orig`) is
% appended by write_GER_xlsx_policy from the .mat workspace, so GER stays
% at 18 columns and GER_nt at 15 columns here.
...
% Policy variant: the `+ sum(D_all_t.*wd*transfer)` term is the policy
% subsidy in the native labor-market aggregation.

Ld_alt    = (1/wd)      * ( sum(((sigma-1)/sigma).*rev_dom.*sdom) + sum(((sigmax-1)/sigmax).*rev_exports.*sdom) + sum(D_imm.*wd.*fe_j) + sum(D_imm.*wd*f_imm) + sum(D_exporter.*wd*f_g) )    + sum(D_all_t .*wd   *transfer);
I_alt     = (1/wimm)    * ( sum((1-sdom).*((sigma-1)/sigma).*rev_dom) + sum((1-sdom).*((sigmax-1)/sigmax).*rev_exports) );

Ld_alt_nt = (1/wd_nt)   * ( sum(((sigma-1)/sigma).*rev_nt.*sdom_nt) + sum(D_imm_nt.*wd_nt.*fe_j_nt) + sum(D_imm_nt.*wd_nt*f_imm_nt) ) + sum(D_all_nt.*wd_nt*transfer);
I_alt_nt  = (1/wimm_nt) * sum((1-sdom_nt).*((sigma-1)/sigma).*rev_nt);

% Trade balance and aggregate ledger of the policy subsidy (subsidy_fixed).
% subsidy_fixed = wd * sum_T (f_j_orig - f_j) * fe_j-scaling + analogous NT term.
% Currently computed but not used in the F-vector (F(11) commented out);

trade_balance = sum(rev_exports_RoW) - sum(rev_exports);
subsidy_fixed = wd   *sum( (f_j_orig    - f_j   ) * (F_t /wd)    .* (((1-sdom)   ./sdom)   .^(1/(iota*(epsilon-1)))) ) ...
              + wd_nt*sum( (f_j_nt_orig - f_j_nt) * (F_nt/wd_nt) .* (((1-sdom_nt)./sdom_nt).^(1/(iota*(epsilon-1)))) );

main_aggregates = [lmd_dd, lmd_xx, trade_balance, Ld_alt-Ld, Ld_alt, I_alt, Ld_alt_nt, I_alt_nt, Lx, Limm_count, Limm_nt_count, Lx_count, Ld_count, Ld_nt_count, sum(rev_exports), sum(rev), sum(rev_nt), subsidy_fixed];
...
%F(11) = subsidy_fixed - total_transfer;                       % Policy variant: would close the subsidy ledger.
